...
export interface SelectOptions {
	...
	/** 外部取消信号：abort 时关闭对话框（等效于 Esc，返回 null） */
	signal?: AbortSignal;
}
...
export async function showSelect<T = string>(
	...
	return ctx.ui.custom<SelectResult<T> | null>(
		(tui, theme, _kb, done) => {
			// ---- 状态 ----
			...
			// ---- 结算守卫 + 外部取消（signal）→ 关闭对话框 ----
			let settled = false;
			const finish = (result: SelectResult<T> | null) => {
				if (settled) return;
				settled = true;
				done(result);
			};
			...
			function handleInput(data: string): void {
				...
				if (matchesKey(data, Key.enter)) {
					...
					if (selIdx < options.length) {
						const opt = options[selIdx];
						finish({ value: opt.value, label: opt.label });
					} else if (allowOther) {
						// "自定义输入" 选项：进入补充输入模式
						state = { ...state, inputMode: true, inputText: '' };
						tui.requestRender();
 ...
 * @param opts.signal 外部取消信号：abort 时关闭对话框并返回 false
 */
export async function showConfirm(
	...
	const result = await showSelect(
		...
		{ detail: message, mode, signal: opts?.signal },
	);
...
export async function showConfirmDestructive(
	...
	return showConfirm(ctx, title, message, 'danger', opts);
}
